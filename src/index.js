export default {
  async fetch(request, env, ctx) {
    ...
    async function rateLimit(path) { if (!env.PAYMENTS_KV) return false; const key = 'rl:' + clientIP + ':' + path.replace(/\//g,'_'); const raw = await env.PAYMENTS_KV.get(key); const count = raw ? parseInt(raw) : 0; if (count >= CONFIG.RATE_LIMIT) return true; await env.PAYMENTS_KV.put(key, String(count + 1), { expirationTtl: 60 }); return false; }
